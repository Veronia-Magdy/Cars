...
if(!isset($_SESSION["logged"]) or !$_SESSION["logged"]){
  ...
  die();
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST"){
        ...
        try{
            $sql = " INSERT INTO `car`( `title`, `content`, `price`, `model`, `type`, `properties`, `imag`) VALUES(?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
    
	$stmt->execute([$title, $content, $price, $model , $type , $properties , $image_name ]);
		...
		<div class="container">
			<form method="POST" action="" class="m-auto" style="max-width:600px" enctype="multipart/form-data">
				...
				<div class="form-group mb-3 row"><label for="select-option1" class="col-md-5 col-form-label">Auto / Manual</label>
					<div class="col-md-7"><select class="form-select custom-select custom-select-lg" name="type" id="select-option1" >
